...
!          write(6,*) var2
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed V(x,y,z)=',stringV
        write(6,*)'V(x,y,z) is from VTRAP_EXT_TD.F'
        stringV='Using defaults from VTRAP_EXT_TD.F'
      else
        write(6,*)statusflag,'in parsed V(x,y,z)=',stringV
      endif
      call destroyfunc()
              case('W(R=|r1-r2|')
                 stringW=Value1
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed W(r1-r2)=',stringW
        write(6,*)'W(R) is from Get_InterParticle.F'
        stringW='Using Defaults from Get_InterParticle.F'
      else
        write(6,*)statusflag,'in parsed W(R)=',stringW
      endif
      call destroyfunc()
              case('Psi_1(x_y_z')
                 stringPSI(1)=Value1
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed =Psi_1',stringPSI(1)
        write(6,*)'Psi_1 is from  Guess_PSI.F'
        stringPSI(1)='Using Defaults from Guess_PSI.F'
      else
        write(6,*)statusflag,'in parsed Psi_1=',stringPSI(1)
      endif
      call destroyfunc()
              case('Psi_2(x_y_z')
                 stringPSI(2)=Value1
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed =Psi_2',stringPSI(2)
        write(6,*)'Psi_2 is from  Guess_PSI.F'
        stringPSI(2)='Using Defaults from Guess_PSI.F'
      else
        write(6,*)statusflag,'in parsed Psi_2=',stringPSI(2)
      endif
      call destroyfunc()
              case('Psi_3(x_y_z')
                 stringPSI(3)=Value1
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed =Psi_3',stringPSI(3)
        write(6,*)'Psi_3 is from  Guess_PSI.F'
        stringPSI(3)='Using Defaults from Guess_PSI.F'
      else
        write(6,*)statusflag,'in parsed Psi_3=',stringPSI(3)
      endif
      call destroyfunc()
              case('Psi_4(x_y_z')
                 stringPSI(4)=Value1
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed =Psi_4',stringPSI(4)
        write(6,*)'Psi_4 is from  Guess_PSI.F'
        stringPSI(4)='Using Defaults from Guess_PSI.F'
      else
        write(6,*)statusflag,'in parsed Psi_4=',stringPSI(4)
      endif
      call destroyfunc()
              case('Psi_5(x_y_z')
                 stringPSI(5)=Value1
      ...
      if(statusflag.ne.'ok') then 
        ...
        write(6,*)statusflag,'in parsed Psi_5=',stringPSI(5)
      endif
      call destroyfunc()
              case('Imprint_MOM')
                 stringMOM=Value1
      ...
      if(statusflag.ne.'ok') then 
        write(6,*)statusflag,' detected in parsed =ImprintMOM',stringMOM
        write(6,*)'Imprint default momentum from Guess_PSI.F'
        stringMOM='Imprint defaults K from Guess_PSI.F'
      else
        write(6,*)statusflag,'in parsed ImprintMOM=',stringMOM
      endif
      call destroyfunc()
              case('Df_cnf_Fock')
