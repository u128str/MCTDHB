#    G N U P L O T
# thisscript  1D_movie_DNS_WK_K.plt
print "GIF usage: gnuplot  -e \"Morb=2; Time_Bgn=0.0; Time_Fnl=11; dt=0.2 \"  thisscript"
print "OUT animated gif: ./media/1D_DNS_WK_K.gif"
...
print "OUT single-shot jpeg: ./media/10.0000000WrkOrbK.jpeg"
print "IN dat: ./DATA/orb_K/10.0000000k-density.dat"
...
fileout1=sprintf("%s%.10stimeWrkOrbK.jpeg",dirOUT,filein1)
...
filein=sprintf("%s%.10sk-density.dat",dirIN,filein1)
fileout1="./media/1D_DNS_WK_K.gif"
...
orbttl(i)=sprintf("Working |PSI_%i|^2(K)",i)
set xlabel "K" font "arial,30"
set ylabel "rho(k,t), Working Orbitals" font "arial,30" 
X1=-10
X2= 10
Y1=-0.1
Y2=Morb*0.75+1
...
set yrange [Y1:Y2]
do for [j=1:n]{
Fulltitle=sprintf("K-space: Working MCTDHB(%i) Orbitals at T=%2.3f",Morb,Time+dt*(j-1))
set title Fulltitle 
filein1=sprintf("%-30.10f",Tmin+(j-1)*dt) 
mvfilein(j)=sprintf("%s%.10sk-density.dat",dirIN,filein1)
print "File namein: ", mvfilein(j)
plot [X1:X2] [Y1:Y2] \
mvfilein(j)  using ($1):($4)  smooth cspl  t sprintf("rho(t=%2.3f)",Tmin+dt*(j-1)) \
 w filledcurves y1=0.0 lc rgb "#00008B" fs transparent solid 0.4 noborder,\
for[i=1:Morb] "" using 1: (column(i+5) +i*0.75) smooth cspl   \
t orbttl(i) w filledcurves y1=i*0.75 lt i  fs trans fs transparent solid 0.8 noborder,\
}
