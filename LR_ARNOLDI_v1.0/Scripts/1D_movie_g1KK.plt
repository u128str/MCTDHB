...
print "OUT single-shot jpeg: media/0.00000000k-correlations.jpeg "
...
filein =sprintf("%s%.10sk-correlations.dat",dirIN,filein1)
fileout1=sprintf("%s%.10sk-correlations.jpeg",dirOUT,filein1)
...
set xlabel "k" font "arial,35"
set ylabel "k'" font "arial,35" 
zero=1.0e-16
...
do for [j=1:n]{
Fulltitle=sprintf(" MCTDHB(%i):  |g^{(1)}(k\',k|t)|^2  at T=%2.3f",Morb,Time+dt*(j-1))
set title Fulltitle 
filein1=sprintf("%-30.10f",Tmin+(j-1)*dt) 
mvfilein(j)=sprintf("%s%.10sk-correlations.dat",dirIN,filein1)
print "File namein: ", mvfilein(j)
splot [minX:maxX][minY:maxY][0:1.01] mvfilein(j) using \
