...
set ylabel "rho(K)" font "arial,30" 

zero=1.0e-16
#============== PSI(i) ======================
filein1=sprintf("%-30.10f",Time)
filein =sprintf("%s%.10sk-density.dat",dirIN,filein1)
fileout=sprintf("%s%.10sk-density.jpeg",dirOUT,filein1)
...
#Fulltitle=sprintf("Working MCTDHB(%i) Orbitals at T=%f",Morb,Time)
Fulltitle=sprintf("MCTDHB(%i) Density in Momentum [K] space at T=%f",Morb,Time)
set title Fulltitle 
...
#plot  [X1:X2] [Y1:Y2] 
plot  [X1:X2] [] \
filein using ($1):($4) t ""  w lp lt -1  pt 5 ps 1.0 ,\
filein using ($1):($4) t dnsttl w filledcurves y1=0.0 lc rgb "#00008B" fs transparent solid 0.4 noborder 

#filein using ($1):(0.02*$5-0.01) t "V(x,t)" w filledcurves y1=-0.01 lc rgb "black" fs transparent solid 0.5 noborder,\
#for[i=1:Morb] "" using 1: ((column(2*i+8)**2+column(2*i+9)**2)/(column(4)*column(4)) +i*0.75) \
