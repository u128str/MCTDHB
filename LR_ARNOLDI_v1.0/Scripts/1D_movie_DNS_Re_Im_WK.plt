#    G N U P L O T
# thisscript 1D_movie_DNS_Re_Im_WK.plt
print "GIF usage: gnuplot  -e \"Morb=2; Time_Bgn=0.0; Time_Fnl=11; dt=0.2 \"  thisscript"
print "OUT animated gif: ./media/1D_DNS_Re_Im_WK.gif"
...
print "OUT single-shot jpeg: ./media/10.0000000timeWrkOrbReIm.jpeg"
...
fileout1=sprintf("%s%.10stimeWrkOrbReIm.jpeg",dirOUT,filein1)
...
filein=sprintf("%s%.10stime.dat",dirIN,filein1)
fileout1="./media/1D_DNS_Re_Im_WK.gif"
...
set ylabel "V(x,t),rho(x,t), Working Orbitals" font "arial,30" 
X1=-10
X2= 10
Y1=-0.1
Y2=Morb*0.75+1
...
for[i=1:Morb] \
"" using 1: (column(2*i+9)/column(4) +i*0.75+0.1) \
t "Im[^{WK}psi_".(i)."]" w filledcurves y1=(i)*0.75+0.1 lt i+1+1  fs trans fs transparent solid 0.5 noborder,\
for[i=1:Morb] \
"" using 1: (column(2*i+8)/column(4) +i*0.75) \
t "Re[^{WK}psi_".(i)."]" w filledcurves y1=i*0.75 lt i+1  fs trans fs transparent solid 0.5 noborder
}
