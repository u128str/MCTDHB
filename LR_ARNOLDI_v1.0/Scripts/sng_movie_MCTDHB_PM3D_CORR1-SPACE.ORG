...
FILE_LIST="`ls *x-correlations.dat | sort -g | sed s/N\$5M\$4x\-correlations.dat//`"
#esac
IFS=$'\n'
((j=0))
for i in ${FILE_LIST}
...
if [ $(echo "$A <= $2"|bc) -eq 1 ]
then
echo "$A""N$5M$4x-correlations.dat" >> TMP 
fi
fi
fi
done
...
splot [$minX:$maxX][$minY:$maxY][0:1.01] \
...
mencoder mf://@qq1.sort -mf w=800:h=600:fps=5:type=jpg -ovc lavc -lavcopts vcodec=msmpeg4v2 -oac copy -o MCTDHB_CORR1_PM3D_X_$1_$2.mpg
echo "Created Movie File is named: MCTDHB_CORR1_PM3D_X_$1_$2.mpg"
