...
rm -f *sort*
  exit 0 ;;
    help|-h*)   
echo "Makes a 1D (|psi_i|^2 Time) movie of evolution of working WK orbitals"
echo "Usage: ../Scripts/sng_movie_MCHB_PSI_WORK 0 15 1 10 "
...
Morb=$4
echo " plot [$minX:$maxX][-0.1:6.0] \
\"$i\" using (\$1):(1*\$5-0.0) t \"V(t)\" w lp lt -1 pt 7 ps 0.0 ,\\">> $i.gnu
ii=1;
x1=0.0000;
while [ $ii -le $Morb ] 
do
...
echo "\"$i\" using (\$1):("$x1"+((\$"$i1"*\$"$i1"+\$"$i0"*\$"$i0"))/(\$4*\$4)) t \"Working Orb |PSI_"$i2"|^2\" w lp lt "$i2" pt 6 ps 0.1 ,\\">>$i.gnu
ii=$[$ii+1]
done 
echo "\"$i\" using (\$1):(\$8) t \"DNS MCTDHB("$Morb")\" w lp lt 4 lw 3 pt 6 ps 0.7  ; " >>$i.gnu
...
mencoder mf://@qq1.sort -mf w=800:h=600:fps=5:type=jpg -ovc lavc -lavcopts vcodec=msmpeg4v2 -oac copy -o PSI_WorK_$1_$2.mpg
#mencoder mf://*.jpg -ovc lavc -lavcopts vcodec=mpeg1video ${FILE_LIST1} -o PSI_$1_$2.avi
echo "Created Movie File is named: PSI_WR_$1_$2.mpg"
