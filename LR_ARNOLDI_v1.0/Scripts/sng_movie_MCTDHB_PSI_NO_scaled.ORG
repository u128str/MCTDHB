...
echo "\"$i\" using (\$1):("$x1"+\$"$ni"*((\$"$i1"*\$"$i1"+\$"$i0"*\$"$i0"))/(\$4*\$4)) t \"NO n("$i2")|PSI_"$i2"|^2\" w lp lt "$i2" lw 5 pt 6 ps 0.1 ,\\">>$i.gnu
ii=$[$ii+1]
done 
echo "\"$i\" using (\$1):(\$8) t \"DNS MCTDHB("$Morb")\" w filledcurves y1=0.0 lc rgb \"red\" fs transparent solid 0.8 noborder  ; " >>$i.gnu
