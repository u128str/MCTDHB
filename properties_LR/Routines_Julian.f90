...
!         if(Morb.eq.1) isft=0 ! for GP less roots
           icnt=0
         DO i=dimL/2-(Morb*Morb+1)-isft+1,dimL/2-(Morb*Morb+1)+Morb
!         wo(i-dimL/2)=w(ind(i-isft))
         ...
         DO i=dimL/2+Morb*Morb+1-Morb+1,dimL/2+Morb*Morb+1-Morb+numeig-(Morb+isft)
             wo(1+Morb+isft+i-(dimL/2+1+Morb*Morb+1-Morb))=w(ind(i))
         indvec(1+Morb+isft+i-(dimL/2+1+Morb*Morb+1-Morb))=ind(i) ! isft=0 for positive ones
         icnt=icnt+1
         END DO
          write(6,*)"Number of desired roots",icnt," actual number ",numeig
         write(6,*)"List of desired roots",indvec
