...
FFTFLAG= -DFFTW=.TRUE. -DSFX1D=FFT_FFTW_1D -DSFX2D=FFT_FFTW_2D  -DSFX3D=FFT_FFTW_3D -DMKLFFT=.FALSE.
...
ifeq ($(FFT),CUDACPP) 
FFTFLAG= -DSFX1D=FFT_CUDACPP_1D -DSFX2D=FFT_CUDACPP_2D  -DSFX3D=FFT_CUDACPP_2D -DMKLFFT=.FALSE. -DFFTW=.FALSE.  -DCUDACPP=.TRUE.
DIRSFFT := ./source/FFTCUDACPP
DIRS1 := 
LIBRARIES = 
CUDA_ALL = -L/common/cuda/lib64 -lcufft -lcudart  -lcublas
endif
...
%.o : %.cu
	nvcc -O3 -arch=compute_20 -c $(FFTFLAG) $< -o $@
