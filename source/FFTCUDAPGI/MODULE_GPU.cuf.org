...
!=======================================
      ...
      USE cufft_m
        ...
        SAVE
         integer :: ierr_gpu
        complex(fp_kind), device, allocatable :: ortkx_d(:), ortky_d(:),ortkz_d(:) ! add oksa ortky_d(:)

!c==========================================================
         contains
         logical function GPU_ALL_init()
         IMPLICIT NONE
         allocate(ortkx_d(NDX),stat=ierr_gpu) 
       if(ierr_gpu /= 0) then 
       write(*,*)"allocation error for ort_kx on GPU"
       GPU_ALL_init=.False.
       else
       write(*,*)"allocation for ortkx_d(NDX) on GPU is OK!!!!"
       GPU_ALL_init=.True.
       endif
        allocate(ortky_d(NDY),stat=ierr_gpu) 
       if(ierr_gpu /= 0) then 
       write(*,*)"allocation error for ort_ky on GPU"
       GPU_ALL_init=.False.
       else
       write(*,*)"allocation for ortky_d(NDY) on GPU is OK!!!!"
       GPU_ALL_init=.True.
       endif
       allocate(ortkz_d(NDZ),stat=ierr_gpu) 
       if(ierr_gpu /= 0) then 
       write(*,*)"allocation error for ort_ky on GPU"
       GPU_ALL_init=.False.
       else
       write(*,*)"allocation for ortkz_d(NDZ) on GPU is OK!!!!"
       GPU_ALL_init=.True.
       endif
