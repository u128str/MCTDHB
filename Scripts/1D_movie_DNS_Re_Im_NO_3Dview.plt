#!/usr/bin/gnuplot 
#    G N U P L O T
# thisscript 1D_movie_DNS_Re_Im_NO_3Dview.plt
print "GIF usage: gnuplot  -e \"Morb=2; Time_Bgn=0.0; Time_Fnl=11; dt=0.2 \"  thisscript"
print "OUT animated gif: ./media/1D_DNS_Re_Im_NO_3Dview.gif"
...
print "OUT single-shot jpeg: ./media/0.00000000timeNatOrbReIm_3Dview.jpeg"
...
fileout1=sprintf("%s%.10stimeNatOrbReIm_3Dview.jpeg",dirOUT,filein1)
...
filein=sprintf("%s%.10stime.dat",dirIN,filein1)
fileout1="./media/1D_DNS_Re_Im_NO_3Dview.gif"
...
zero=1.0e-16
do for [j=1:n]{
Fulltitle=sprintf("Re and Im parts of natural MCTDHB(%i) Orbitals at T=%2.3f",Morb,Tmin+dt*(j-1))
set title Fulltitle 
...
mvfilein(j) using (0.02*$5-0.01):1:(0.0) t "V(x,t)" w lp lt -1 pt 7 ps 0.0 lw lwv,\
        ""  using ($8):1:(0.0)  t dnsttl w lp lt -1 lw lwv pt 6 ps 0.7,\
for[i=1:Morb] \
"" using (column(2*Morb+2*(i-1)+10)/column(4) +(Morb-i+1)*scl):1:(column(2*Morb+2*(i-1)+11)/column(4) +(Morb-i+1)*scl) \
