...
print "OUT single-shot jpeg: ./media/10.0000000timeNatOrb.jpeg"
...
fileout1=sprintf("%s%.10stimeNatOrb.jpeg",dirOUT,filein1)
...
orbttl(i)=sprintf("|^{NO}PSI_%i|^2",Morb-i+1)
set xlabel "x" font "arial,14"
set ylabel "V(x,t),rho(x,t),Natural Orbitals" font "arial,14" 
X1=-10
X2= 10
Y1=-0.1
Y2=Morb*0.75+1
...
set yrange [Y1:Y2]
do for [j=1:n]{
Fulltitle=sprintf("Natural MCTDHB(%i) Orbitals at T=%2.3f",Morb,Time+dt*(j-1))
set title Fulltitle 
...
for[i=1:Morb] "" using 1:((column(2*Morb+2*(i-1)+10)**2+column(2*Morb+2*(i-1)+11)**2)/(column(4)*column(4)) +(Morb-i+1)*0.75) \
