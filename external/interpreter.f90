...
!	integer, parameter	 :: realkind = selected_real_kind(p=13,r=200)		
	...
	character(len=1255),  dimension(:), allocatable, private	:: stokens
	...
	character(len=1255),								private :: toke
...
contains
	...
	else if (trim(toke) == 'hvsd') then ! Added by STR & SK 01/09/14
		itoke = itoke + 1
		toke = stokens(itoke)
		call brackets()
		operations(ioperations) = 29
		ioperations = ioperations + 1
	...
	integer											 :: dt = 1
	integer											 :: i 	
        real*8, external:: sind,cosd,tand
        double precision:: hvsd

!       include  "sind_cosd_ect.inc"
	...
	select case(operations(i))
		...
		case (29)
			pdata(st) = hvsd(pdata(st))
		case default
			st = st + 1
			pdata(st) = vars(operations(i)-29)
		end select
	end do
...
do i = 1, nchar-1
  do j = 1, 36
    if(funcao(i:i) == '0' .or. funcao(i:i) == 'n' .or. &
      funcao(i:i) == 's' .or. funcao(i:i) == 'h' .or. funcao(i:i) == 'd' .or. funcao(i:i) == 'g' .or. &
